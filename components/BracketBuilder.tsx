  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-white selection:bg-sky-500/30">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[1400px] flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-6 rounded-3xl bg-white/5 p-8 shadow-2xl ring-1 ring-white/10 backdrop-blur-xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
            <div className="flex flex-1 items-center gap-4 w-full">
              <Link 
                href="/dashboard"
                className="flex items-center justify-center shrink-0 rounded-xl bg-white/5 p-3 text-slate-400 hover:bg-white/10 hover:text-white transition-colors border border-white/10"
              ...
              <input
                ...
                className="w-full bg-transparent text-3xl sm:text-4xl font-black tracking-tighter text-white outline-none transition-colors placeholder:text-white/20 focus:placeholder:text-white/10"
              />
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Tamanho da Chave</h2>
            </div>
            <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    ...
                    className={`group relative flex items-center justify-center overflow-hidden rounded-xl border-2 px-5 py-2.5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 active:scale-95 ${
                      isSelected
                        ...
                        : "border-white/5 bg-white/5 shadow-sm hover:border-sky-500/30"
                    }`}
                  >
                    {isSelected && (
                    ...
                    >
                      {size.value}
                      <span className={`text-[10px] font-bold uppercase tracking-widest ${isSelected ? 'text-sky-400' : 'text-slate-500'}`}>
                        {size.label}
                      </span>
                    </span>
                  </button>
              ...
              {/* Opção Customizada */}
              <div className="flex items-stretch overflow-hidden rounded-xl border-2 border-white/10 bg-white/5 shadow-sm transition-all focus-within:border-sky-500/50 focus-within:ring-4 focus-within:ring-sky-500/10">
                <input
                  type="number"
                  min="2"
                  step="2"
                  ...
                  className="w-24 bg-transparent px-3 py-2 text-center text-sm font-bold text-white outline-none placeholder:text-white/20"
                />
                <button
                  type="button"
                  onClick={handleCustomSize}
                  className="border-l-2 border-white/10 bg-white/5 px-3 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 transition-colors hover:bg-white/10 hover:text-white active:bg-white/20"
                >
                  OK
                </button>
              </div>
        ...
        <section className="w-full">
          <div className="overflow-hidden rounded-3xl bg-white/5 p-6 shadow-2xl ring-1 ring-white/10 backdrop-blur-xl">
            {rounds.length > 0 ? (
              <BracketViewer 
                rounds={rounds} 
              ...
              <div className="flex min-h-[420px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-white/10 bg-white/5 p-6 text-center">
                <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white/5 text-slate-500 ring-1 ring-white/10">
                  <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                  </svg>
                </div>
                <h3 className="text-lg font-bold text-white">Nenhum torneio criado</h3>
